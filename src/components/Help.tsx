...
function VwapArt() {
  return (
    <svg className="art small" viewBox="0 0 260 80" role="img" aria-label="VWAP">
      <polyline points="10,52 30,38 50,58 70,30 90,46 110,24 130,40 150,20 170,34 190,16 210,30 230,14 250,26"
        fill="none" stroke={DIM} strokeWidth="1" />
      <polyline points="10,54 50,48 90,42 130,36 170,32 210,28 250,26" fill="none" stroke="#ff5ec8" strokeWidth="2" />
      <text x="10" y="14" fill={UP} fontSize="9">上＝その日に買った人の平均より高い</text>
      <text x="10" y="76" fill="#ff5ec8" fontSize="9">VWAP＝その日ここまでの売買代金 ÷ 出来高</text>
    </svg>
  );
}
      ...
      },
      {
        ...
        body: (
          <>
            ...
            <p>一括取消の上の<b>「指値注文」</b>を押すと小窓が開き、売買・新規/返済・値段・株数を打ち込んで注文できます。ダブルクリックと同じ注文を、値段を指定して出すためのものです。出している注文の一覧と取消もここでできます。小窓は見出しを掴んで移動、右下をつまんで大きさ変更、「—」で帯に畳めます。</p>
          </>
        ),
      },
      {
        ...
        body: (
          <>
            <p>自分で出す注文は<b>指値</b>（値段を指定する注文）です。歩み値がその値段に触れたら成立します。</p>
            <p>買い注文は指定より<b>安い</b>値段が付いたとき、売り注文は<b>高い</b>値段が付いたときに成立します。いまの値段より有利でない側に置けば、その場ですぐ成立します。</p>
            <p>一括取消の上の<b>「逆指値」</b>の小窓では、<b>損切り（逆指値）</b>と<b>利確</b>を別々にオンにできます。オンの間に出した新規注文すべてに自動の返済が付きます。損切りは建値から設定した幅だけ<b>不利な</b>値段の逆指値で、歩み値がそこに触れたら<b>そのときの値段で</b>返済します（値段が飛べばそのぶん滑ります。板では<b>点線のチップ</b>）。利確は建値から幅ぶん<b>有利な</b>値段の普通の指値です。オフにしても出ている注文は残るので、外したいときは小窓の一覧から取り消します。</p>
          </>
        ),
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        body: (
          <>
            <p>その日の寄り付きからの<b>売買代金 ÷ 出来高</b>、つまり「その日に売買が成立した値段の平均」です。移動平均と違って本数の指定はなく、寄りからの累計で1本引きます。</p>
            <p>その日に買った人の平均建値にあたるので、<b>上にいれば買い方が有利、下にいれば売り方が有利</b>と読みます。機関投資家がこの線を目安に売買するため、押し目や戻りの目標にされやすい線です。</p>
          </>
        ),
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        body: (
          <>
            <p><b>過去のチャレンジを、そのときの売買ごと見直す機能です。</b>フッターの「CSVを開く」の右にある <b>⏱ リプレイ</b> から、<b>チャレンジID-銘柄-日付</b> の一覧で選びます。</p>
            <p>選ぶとそのチャレンジで使ったCSVに切り替わり、<b>チャレンジを始めた時刻まで巻き戻ります</b>。あとは通常どおり ▶ で再生してください。再生が進むと、そのとき自分が売買した場所に<b>矢印と値段</b>が出て、<b>建てた値段から返した値段まで1本の線</b>が引かれます（勝ちは赤・負けは青）。「どのタイミングで、どの値段で入って、どうなったか」がそのまま見えます。</p>
            <p>先の値動きを教えてしまわないよう、<b>まだ再生していない足の売買は出しません</b>。再生が追いつくにつれて現れます。</p>
            <p>リプレイ中は<b>チャートが赤枠</b>になり、左上に見ているチャレンジが出ます。通常の再生と取り違えないための印です。<b>CSVを選び直すか、赤枠の ✕ を押すと元に戻ります。</b></p>
            <p className="warn">※ 記録中にリプレイへ入ると、そのチャレンジはそこで区切って保存されます（巻き戻して見ることになるため）。</p>
          </>
        ),
      },
      {
      ...
      },
      {
        ...
        body: (
          <>
            ...
            <p>音・配色・昼休みスキップは、説明ボタンの右の <b>⚙ 設定</b> にまとまっています。<b>配色は18から選べます</b>（midnight・spring・summer・fall・winter・crazy rainbow・wild jungle・ghost mansion・happy purple・japan・usa・god zeus・rising sun・casino・death metal・egypt・atlantis・random）。spring・winter・happy purple・japan・rising sun は白地です。<b>random は押すたびに引き直し</b>で、白地になることもあります。チャートも板も歩み値も同時に切り替わります。</p>
      ...
      },
      {
        ...
        body: (
          <>
            <p>ヘッダー右、<b>戦略</b>のとなりの<b>日報</b>から、その日の振り返りを書き残せます。<b>日付・タイトル</b>と、<b>改善リスト・メモ</b>の2つの欄があります。</p>
            <p>「画像を追加」のほか、チャートのスクリーンショットを<b>ドロップ</b>したり<b>貼り付け（⌘V）</b>したりしても添えられます。添えた画像は<b>保存</b>を押したときにまとめて置かれ、<b>取消</b>で元に戻せます。</p>
            <p>左の日付一覧から過去の日報を開いて読み返し、<b>編集</b>で書き直せます。中身は public/data/diary/日付/ にテキスト（entry.md）と画像のまま置かれるので、エディタでもそのまま読めます。</p>
          </>
        ),
      },
    ],
